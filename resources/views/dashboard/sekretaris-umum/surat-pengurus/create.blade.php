...
<x-dashboard.page-wrapper :title="$title"
    ...
    <div x-data="qrPlacement({
        sekretarisUmumId: {{ $users->where('role_id', 3)->where('is_active', true)->first()->id ?? 'null' }},
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6">
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code') }}" required minlength="3"
                            ...
                            title="Kode surat harus berisi 3-100 karakter, hanya huruf, angka, spasi, tanda hubung, titik, dan garis miring"
                    ...
                    <div class="col-span-6">
                        <label for="date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tanggal
                            Surat <span class="text-red-500">*</span></label>
                        <input type="date" name="date" id="date" value="{{ old('date') }}" required min="2000-01-01"
                            max="2100-12-31"
                            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                        @error('date')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
        ...
        return {
            ...
            signers: {
                sekretaris_umum: {
                    ...
                    fullname: @json($users->where('role_id', 3)->where('is_active', true)->first()->fullname ?? '-'),
                    order: 1,
                    qr_page: null,
                    qr_x: null,
                    qr_y: null,
                    isPlaced: false,
                    isActive: @json($users->where('role_id', 3)->where('is_active', true)->first() ? true : false)
                },
                ketua_umum: {
                    id: ketuaUmumId,
                    label: 'KU',
            ...
            validateAndSubmit(e) {
                ...
                if (!/^[a-zA-Z0-9\s\-\.\/]+$/.test(codeValue)) {
                    showCustomAlert('danger', 'Validasi Gagal', ['Kode surat hanya boleh berisi huruf, angka, spasi, tanda hubung, titik, dan garis miring']);
                    return;
                }
                ...
                if (!ketuaUmum || !ketuaUmum.isPlaced) {
                    showCustomAlert('danger', 'Validasi Gagal', ['QR untuk penandatangan Ketua Umum wajib ditempatkan']);
                    return;
                }
